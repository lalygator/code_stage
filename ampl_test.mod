...
# param M := round(2*OWA*1.2);
param M := round(2*OWA);

# Echantillonnage dans le plan focal
# param dxi := 1.2*OWA/M;
# param deta := 1.2*OWA/M;
...
read {{x2 in X2s, y2 in Y2s}} MTF[x2,y2] < fits/quad_MTF_ELT_circ.fits;

# Creation des variable dans le plan pupille
var C_pup {{x2 in X2s, eta in Etas}};
var E_pup {{x2 in X2s, y2 in Y2s}};
var E00_pup;

subject to C_pup_def {{x in X2s, eta in Etas}}: C_pup[x2,eta] = 2*sum {{xi in Xis: (xi,eta) in E}} E[xi,eta]*cos(2*pi*x2*xi)*dxi;
...
subject to E00_pup_def : E00_pup = sum {{(xi,eta) in E}} 4*E_pup[xi,eta]*dxi*deta; 
# mais c'est utile ?
...
subject to C_foc_def {{xi in Xis, y2 in Y2s}}: C_foc[xi,y2] = 2*sum {{x2 in X2s}} E_pup[x2,y2]*cos(2*pi*x2*xi)*dx2;
subject to E_foc_def {{xi in Xis, eta in Etas}}: E_foc[xi,eta] = 2*sum {{y2 in Y2s}} C_foc[xi,y2]*cos(2*pi*y2*eta)*dy2;

subject to E00_foc_def : E00_foc = sum {{(x2, y2)}} 4*E_foc[x2,y2]*dx2*dy2; 
...
subject to sidelobe_DZ_pos {{(xi,eta) in DH}}:  E[xi,eta]^2 <= c 
# *0.4*sqrt(xi^2+eta^2)^(-3/2)+T_constraint*1e-3; 

# subject to sidelobe_DZ_neg {{(xi,eta) in DH}}:  E[xi,eta]^2 >= -c # *0.4*sqrt(xi^2+eta^2)^(-3/2)+T_constraint*1e-3;
...
# On appelle l'algo de résolution pour résoudre itérativmeent le problème
