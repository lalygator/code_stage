...
# On passe le champ électrique dans un plan pupille avec une autre TF pour la multiplier à la MTF
#Echantillonage de la pupille (taille d'un pixel)
param N2 := M;
param dx2 := 1/M;
param dy2 := 1/M;
...
subject to C_pup_def {x2 in X2s, eta in Etas}: C_pup[x2,eta] = 2*sum {xi in Xis} E[xi,eta]^2*cos(2*pi*x2*xi)*dxi;
subject to E_pup_def {x2 in X2s, y2 in Y2s}: E_pup[x2,y2] = 2*sum {eta in Etas} C_pup[x2,eta]*cos(2*pi*y2*eta)*deta;

# repassage dans le plan focal
var C_foc {xi in Xis, y2 in Y2s};
var E_foc {xi in Xis, eta in Etas};

subject to C_foc_def {xi in Xis, y2 in Y2s}: C_foc[xi,y2] = 2*sum {x2 in X2s} E_pup[x2,y2]*cos(2*pi*x2*xi)*dx2*MTF[x2,y2];
