...
def PSF(Ein, fov, N): #optique = pupil, # N = nbr_pix
    #image = (ft_BASIC(SQ(optique), 1132 / 1024, fov, N, direction=1).real)**2
    #return (ft_BASIC_phase(SQ(optique), 1132 / 1024, fov, N, 1,1e-6,A).real)**2
    return (ft_BASIC(Ein, 1132 / 1024, fov, N, direction=1).real)**2

def PSFs(p,m,fov,Ny,N,owa,ecran):#permet d'avoir la psf moyenne de m écran de phase combiné à la pupille p
    size = int(2.5*owa*2*Ny) # taille de la matrice de signal
...
def OTF(psf,fov,N): #soit le produit de OTF_telescope et OTF_atm, ou autocorrelation onde incidente capturée par le telescope
    return ft_BASIC(psf,1132 / 1024, fov, N, direction=1)

def DSP(psf_LP,fov,N,otf): # ? Est ce que le plot de la DPS est correct ?
    dsp = ft_BASIC(np.log(ft_BASIC(psf_LP,1132 / 1024, fov, N,direction=-1)/otf),1132 / 1024, fov, N,direction=-1)
    dsp -= np.max(dsp)
    return dsp
